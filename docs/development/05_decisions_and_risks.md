...
| R011 | local/flow domain 太小，人工邊界主導 entry/exit KDE | M/H | 本期 A 採 v3 原 bbox、12.5 km receptor core 與 20 km local domain；兩站共用 outer stop，local domains 可重疊。需以 OCM native、OCM surface、NWW analysis 的實際共同 forcing margin 驗證，舊 v4 南擴與 35 km sensitivity 僅作歷史背景 | 三產品在 20 km local-to-outer margin 未共同保留至少兩個有效格點、邊界證據未完成、主要指標不穩定或短時同邊退出集中時，阻擋 A formal；不得以 receptor native 篩選或舊 v4 margin 代替 |
...
| R022 | 只利用單一 forcing source margin，含 Stokes 軌跡卻超出 NWW analysis 支撐 | M/H | A 本期 policy 同時驗證 OCM native、OCM surface、NWW analysis 與逐時 mask 的 20 km local-to-outer margin；B-D 保留原 `expanded_domain` 敏感度，均須依各自三產品支援驗證 | 任一必要 forcing 在 local/outer margin 無效即阻擋受影響 version，不以 current-only 結果冒充 baseline |
