...
計畫書中「每一處開放海域高達 1,000 組」與後續明列的 `10 × 20 × 50 = 10,000` 相互矛盾。依已記錄的使用者裁決，五個獨立研究站點各自採 10,000 個基礎情境：貢寮、龜山島、新竹、南灣、連江合計 50,000 個。每個情境若配置 `M` 個隨機系集成員，總軌跡數為各情境成員數之和；所有情境使用同一 `M` 時為 `50,000 × M`。C 區 forcing 產品仍使用歷史識別碼 `houwan_nmmba_cache_v3`，但現行站點識別碼固定為 `nanwan`。裁決依據與完整需求文字見[需求追溯與範圍裁決](../foundation/01_requirements_traceability.md)。
...
| 資料與幾何 | `gap_policy.py` | 已核定的不可補值原則與粒子停止狀態 | 集中定義第一缺口截尾、沉底整點可用性與有效統計分母的版本化識別碼，供設定、輸入、執行與報告共同引用。 | `OBSERVED_GAP_CENSORED_STOP_AT_FIRST_GAP_POLICY_ID` |
...
| 資料與幾何 | `input_derivation.py`、`input_horizon.py` | 已驗收產品、共同支援日數與逐筆到達時刻 | 建立不可覆寫的共同輸入；legacy 重新核對完整回溯窗，新版則列舉缺口與第一個向後缺口，同一母體可綁定不同執行日數，保留共同情境身分。 | `build_input_derivatives`、`validate_generic_gap_payload`、`create_release_config` |
| 資料與幾何 | `horizon_suite.py` | 未綁定設定 template、horizons 回溯日數，以及已接受的 OCM schema 3 `ocm_native`／`ocm_surface` 與 NWW3 schema 1 `nww3_analysis` 產品根目錄 | 以最大 horizon 建立一份共同母體，`inputs-build` 僅執行一次，再由同一份 `common-input` 產生多份 release config、validation 與 manifest／SHA-256；legacy 要求 gap-safe，新版完整列舉缺口並在第一缺口截尾，兩者都不得補零或用最近值替代。 | `build_horizon_suite`、`validate_horizon_suite` |
...
| 資料與幾何 | `receptors.py`、`arrival_times.py` | 可長期濕潤的網格面、各時段資料品質指標、master seed 與 face provenance | current formal 先建立通過 static-ocean／50-arrival persistent-wet／forcing gate 的候選池，再以每站 seeded random 無放回抽 5 個水平 face；每面另抽 4 個 `(0,1)` normalized vertical fractions，以及 48 個分層時刻加 2 個事件時刻。歷史 loader 才使用 maximin 與固定 10/40/70/near-bed helper。 | `select_horizontal_receptors_random_from_pool`、`sample_random_vertical_draws`、`_build_face_vertical_support`、`select_arrival_times` |
